...
[`November iteration plan](https://github.com/microsoft/vscode/issues/110690),
we continued to focus for two weeks on housekeeping GitHub issues and pull
requests as documented in our
...
A new setting `workbench.editor.splitOnDragAndDrop` prevents editor group
...
- `editorOrder` - Editors are listed in the same order as editor tabs are shown
  (default).
...
Diff editors opened from the Source Control view will now reveal their file path
...
Whenever there isn't enough space to fit all views inside the workbench, VS Code
...
- `git.pruneOnFetch` - Make VS Code run `git fetch --prune` when fetching remote
  refs.
...
- `git.useCommitInputAsStashMessage` - Enable VS Code to use the commit message
  in the source control input box as a stash message, when running **Git:
  Stash**.
- `git.followTagsWhenSync` - Follow tags when running **Git: Sync**.
- `git.checkoutType` - Control what refs are shown, and in what order, when you
...
- `${pathSeparator}` - Resolves to the character used by the operating system to
...
An extension's details page now shows a context menu with all of the actions for
...
You can start extension bisect via the **Help: Start Extension Bisect** command
...
VS Code will now include basic `@see` references while performing renames. You
...
- `html.format.templating`: Honor django, erb, handlebars, and php templating
  language tags.
- `html.format.unformattedContentDelimiter`: Keep text content together between
  this string.
...
The new `fs.isWritableFileSystem` API lets you check if a given file system
...
read the linked issue for additional context, and participate if you have input
to give. We expect to ship the provider portion of the API significantly earlier
...
In this milestone, the conditional exception
...
Watch the recording of developers Alex Ross and Benjamin Pasero's session at
...
- [`@alisonnoyes](https://github.com/alisonnoyes): Quiet Light theme diff syntax
  highlighting [PR #107926`](https://github.com/microsoft/vscode/pull/107926)
...
- [`Borja Zarco (@bzarco)](https://github.com/bzarco): Don't fail when resolving
  config variables without a `folderUri`.
...
- [`John Murray (@gjsjohnmurray)](https://github.com/gjsjohnmurray)
    ...
    - fix #106487 implement 'enablement' clause for viewsWelcome contribution's
      buttons [`PR #107705`](https://github.com/microsoft/vscode/pull/107705)
...
- [`Dhruva Srinivas (@IQUBE-X)](https://github.com/IQUBE-X): Add Definition list
  to Markdown Snippets
...
- [`Oliver Larsson (@LarssonOliver)](https://github.com/LarssonOliver):
  ...
  [PR #89249`](https://github.com/microsoft/vscode/pull/89249)
...
- [`Shivang Tripathi (@shivangg)](https://github.com/shivangg): feat: Autodetect
  npm.packageManager #102050
  [PR #102494`](https://github.com/microsoft/vscode/pull/102494)
- [`Shunsuke Iwamoto (@shskwmt)](https://github.com/shskwmt): Fix #93946. Remove
...
- [`Suman B K (@sumanbk92)](https://github.com/sumanbk92): Including
  ` as part of foldEndPairCharacters [PR #110108`](https://github.com/microsoft/vscode/pull/110108)
- [`Tim (@Timmmm)](https://github.com/Timmmm): Add atomic tabs option
...
- [`Alexander (@usernamehw)](https://github.com/usernamehw)
    - Add a setting to include word based suggestions regardless of the language
      [`PR #110494`](https://github.com/microsoft/vscode/pull/110494)
...
- [`Jean Pierre (@jeanp413)](https://github.com/jeanp413): Fix autocomplete text
  edit outputs </[object Object]>
  [PR #93`](https://github.com/microsoft/vscode-html-languageservice/pull/93)
...
- [`Fabian Meumertzheim (@fmeum)](https://github.com/fmeum): Ensure package.json
  is writable in \*.vsix
