...
The Language Model API supports two types of messages when building the language
model prompt:
...
> **Note**: Currently, `gpt-4o` is supported for the language model family. We
> expect that the list of supported models grows over time. `gpt-3.5` and
> `gpt-4` are deprecated.
