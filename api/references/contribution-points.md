...
able to set these configuration options in the Settings editor or by editing a
...
Note that if the extension has multiple categories of extensions, and the title
of one of the categories is the same as the extension display name, then the
settings for that category will be placed directly below the main extension
...
The `properties` 2️⃣ in your configuration will be a dictionary of configuration
properties.

In the settings UI, your configuration key will be used to namespace and
construct a title. Though an extension can contain multiple categories of
settings, each setting of the extension must still have its own unique key.
Capital letters in your key are used to indicate word breaks. For example, if
your key is `gitMagic.blame.dateFormat`, the generated title for the setting
will look like this:

> Blame: **Date Format**

Entries will be grouped according to the hierarchy established in your keys. So
...
Otherwise, properties without an explicit order field appear in alphabetical
order (**not** the order in which they're listed in the manifest).
...
For `boolean` entries, the `description` (or `markdownDescription`) will be used
...
sorted in increasing alphabetical order within the settings UI.
...
your descriptions will be parsed as Markdown. \
...
You can also contribute default editor configurations for the provided language.
...
Contributes and defines an object literal structure that allows to uniquely
...
When defined, the profile will show up in the terminal profile selector. When
